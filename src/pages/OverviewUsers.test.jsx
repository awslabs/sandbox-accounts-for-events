...
test("renders OverviewUsers, enters valid and invalid texts, submits", async () => {
    ...
    const fetchLeasesAction = vi.spyOn(leaseActions, "fetchLeases").mockImplementation((leases) => () => leases)
    const fetchUsersAction = vi.spyOn(userActions, "fetchUsers").mockImplementation((users) => () => users)
    ...
    const searchInputElement = screen.getByPlaceholderText(/find users/i);
    ...
    expect(fetchUsersAction).toHaveBeenCalled()
